...
class Master extends DBConnection
{
	...
	public function save_response()
	{
		...
		if (isset($ins) && count($ins) == count($question)) {
			$this->settings->set_flashdata("Let's go", " Data berhasil disimpan");
			return 1;
		} else {
			return 2;
			exit;
	...
	public function delete_response()
	{
		...
		$del = $this->conn->query("DELETE FROM `questions` where id = $id");
		if ($del) {
			$this->settings->set_flashdata("Let's go", " Data berhasil di dihapus");
			return 1;
		} else {
			$this->conn->error;
		}
	...
	public function delete_unanswer()
	{
		...
		if ($del) {
			$this->settings->set_flashdata("Let's go", " Data berhasil dihapus");
			return 1;
		} else {
			$this->conn->error;
		}
